...
/// @notice Source-chain canonical registry for permissioned bank-chain validator epochs.
/// @dev Destination clients do not mutate their remote validator view by admin sync. They
///      import these source-originated epoch artifacts only when certified by the currently
///      trusted source epoch.
contract BankValidatorSetRegistry is AccessControl {
    bytes32 public constant VALIDATOR_EPOCH_PRODUCER_ROLE = keccak256("VALIDATOR_EPOCH_PRODUCER_ROLE");
    ...
    event BankValidatorEpochCommitted(
        ...
        returns (ValidatorEpoch memory epoch)
    {
        ...
        if (oldEpochId != 0) {
            emit BankValidatorEpochCommitted(
                sourceChainId,
                oldEpochId,
