...
/// @title EVMProofTypes
/// @notice Ethereum account and storage proof types used by the institutional gateway.
library EVMProofTypes {
    struct StorageProof {
    ...
    function rlpEncodeWord(bytes32 word) internal pure returns (bytes memory) {
        ...
        if (length == 1 && uint8(value) < 0x80) return abi.encodePacked(bytes1(uint8(value)));
