...
contract PolicyControlledVoucherToken is ERC20, AccessControl, Pausable {
    ...
    function mintWithPolicy(address to, address canonicalAsset_, uint256 sourceChainId, uint256 amount, bytes32 packetId)
        external
        onlyRole(APP_ROLE)
        whenNotPaused
    {
        ...
        require(canonicalAsset_ != address(0), "ASSET_ZERO");
        require(canonicalAsset == canonicalAsset_, "CANONICAL_ASSET_MISMATCH");
        require(amount > 0, "AMOUNT_ZERO");
        require(packetId != bytes32(0), "PACKET_ZERO");
        require(!processedMintPackets[packetId], "MINT_PACKET_PROCESSED");

        (bool allowed, bytes32 code) = policyEngine.canMintVoucher(sourceChainId, to, canonicalAsset_, amount);
        if (!allowed) revert PolicyDenied(code);

        processedMintPackets[packetId] = true;
        _mint(to, amount);
        policyEngine.noteVoucherMinted(sourceChainId, to, canonicalAsset_, amount);
        emit VoucherMinted(to, canonicalAsset_, amount, sourceChainId, packetId);
    }

    function burnFrom(address from, uint256 amount) external onlyRole(APP_ROLE) whenNotPaused {
    ...
    function burnFromWithPolicy(address from, address canonicalAsset_, uint256 amount) external onlyRole(APP_ROLE) whenNotPaused {
        require(canonicalAsset_ != address(0), "ASSET_ZERO");
        require(canonicalAsset == canonicalAsset_, "CANONICAL_ASSET_MISMATCH");
        require(from != address(0), "FROM_ZERO");
        require(amount > 0, "AMOUNT_ZERO");
        _burn(from, amount);
        policyEngine.noteVoucherBurned(from, canonicalAsset_, amount);
        emit VoucherBurned(from, amount);
    }
