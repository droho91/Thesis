...
import {IBCPacketStoreSlots} from "../core/IBCPacketStoreSlots.sol";
import {IBCPacketLib} from "../core/IBCPacketLib.sol";

/// @title PacketProofBuilder
/// @notice Test-only helper that constructs small synthetic EVM storage tries for IBC packet tests.
contract PacketProofBuilder {
    ...
    function buildPacketStorageProof(address packetStore, IBCPacketLib.Packet calldata packet)
        external
        pure
        returns (BuiltPacketStorageProof memory built)
    {
        built.leafSlot = IBCPacketStoreSlots.packetLeafAt(packet.sequence);
        built.pathSlot = IBCPacketStoreSlots.packetPathAt(packet.sequence);

        bytes32 leaf = IBCPacketLib.leafHash(packet);
        bytes32 path = IBCPacketLib.commitmentPath(packet);
