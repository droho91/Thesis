...
import {IBCConnectionKeeperV2} from "./IBCConnectionKeeperV2.sol";

/// @title IBCChannelKeeperV2
/// @notice Minimal channel keeper for the v2 rebuild lane.
/// @dev This intentionally gates the existing address-port packet shape while keeping channel state explicit.
contract IBCChannelKeeperV2 is AccessControl, IBCEVMProofBoundaryV2 {
    bytes32 public constant CHANNEL_ADMIN_ROLE = keccak256("CHANNEL_ADMIN_ROLE");
    bytes32 public constant CHANNEL_COMMITMENT_TYPEHASH = keccak256("IBCLite.ChannelEnd.v2");

    uint256 internal constant CHANNEL_COMMITMENTS_SLOT = 3;

    uint256 public immutable localChainId;
    IBCConnectionKeeperV2 public immutable connectionKeeper;
    ...
    constructor(uint256 _localChainId, address connectionKeeper_, address admin)
        IBCEVMProofBoundaryV2(address(IBCConnectionKeeperV2(connectionKeeper_).besuLightClient()))
    {
        ...
        connectionKeeper = IBCConnectionKeeperV2(connectionKeeper_);
