...
import {IBCChannelKeeperV2} from "./IBCChannelKeeperV2.sol";
import {IBCEVMTypesV2} from "./IBCEVMTypesV2.sol";
import {IBCPacketHandlerSlotsV2} from "./IBCPacketHandlerSlotsV2.sol";
import {IBCPacketStoreV2} from "./IBCPacketStoreV2.sol";
import {IBCPacketLibV2} from "./IBCPacketLibV2.sol";
import {IBCProofVerifierV2} from "./IBCProofVerifierV2.sol";
import {
    IBCPacketAcknowledgementReceiverV2,
    IBCPacketReceiverV2,
    IBCPacketTimeoutReceiverV2
} from "./IBCPacketReceiverV2.sol";

/// @title IBCPacketHandlerV2
/// @notice Minimal v2 packet handler with receipts and acknowledgements, anchored on BesuLightClient.
contract IBCPacketHandlerV2 is AccessControl, IBCProofVerifierV2 {
    bytes32 public constant CLIENT_ADMIN_ROLE = keccak256("CLIENT_ADMIN_ROLE");

    uint256 public immutable localChainId;
    IBCChannelKeeperV2 public immutable channelKeeper;
    ...
    constructor(uint256 _localChainId, address besuLightClient_, address channelKeeper_, address admin)
        IBCProofVerifierV2(besuLightClient_)
    {
        ...
        channelKeeper = IBCChannelKeeperV2(channelKeeper_);
    ...
    function recvPacketFromStorageProof(
        ...
        bytes memory acknowledgement = IBCPacketReceiverV2(packet.destination.port).onRecvPacketV2(packet, packetId);
    ...
    function acknowledgePacketFromStorageProof(
        ...
        IBCEVMTypesV2.StorageProof calldata acknowledgementProof
    ) external returns (bytes32 packetId, bytes32 acknowledgementHash) {
        ...
        require(
            acknowledgementProof.storageKey == IBCPacketHandlerSlotsV2.acknowledgementHash(packetId),
            "ACK_STORAGE_KEY_MISMATCH"
        );
        require(
            keccak256(acknowledgementProof.expectedValue) ==
                keccak256(IBCEVMTypesV2.rlpEncodeWord(acknowledgementHash)),
        ...
        if (application != address(0)) {
            IBCPacketAcknowledgementReceiverV2(application).onAcknowledgementPacketV2(
    ...
    function timeoutPacketFromStorageProof(
        IBCPacketLibV2.Packet calldata packet,
        address remotePacketHandler,
        IBCEVMTypesV2.StorageProof calldata receiptAbsenceProof
        ...
        require(
            receiptAbsenceProof.storageKey == IBCPacketHandlerSlotsV2.packetReceipt(packetId),
        ...
        address application = portApplications[packet.source.port];
        if (application != address(0)) {
            IBCPacketTimeoutReceiverV2(application).onTimeoutPacketV2(packet, packetId);
        }
    ...
    function _requireLocalPacketCommitment(bytes32 packetId) internal view {
        ...
        require(IBCPacketStoreV2(localPacketStore).committedPacket(packetId), "PACKET_NOT_COMMITTED");
    }

    function _packetTimedOut(IBCPacketLibV2.Packet calldata packet, uint256 trustedHeight)
        internal
        view
        returns (bool)
    {
