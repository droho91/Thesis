...
import {IBCPacketAcknowledgementReceiverV2, IBCPacketReceiverV2, IBCPacketTimeoutReceiverV2} from
    "../core/IBCPacketReceiverV2.sol";
import {IBCPacketLibV2} from "../core/IBCPacketLibV2.sol";
import {IBCPacketStoreV2} from "../core/IBCPacketStoreV2.sol";
import {PolicyControlledEscrowVaultV2} from "./PolicyControlledEscrowVaultV2.sol";
import {PolicyControlledVoucherTokenV2} from "./PolicyControlledVoucherTokenV2.sol";

/// @title PolicyControlledTransferAppV2
/// @notice Policy-aware v2 packet application that bridges the transport lane to voucher and escrow actions.
contract PolicyControlledTransferAppV2 is
    AccessControl,
    IBCPacketReceiverV2,
    ...
    IBCPacketTimeoutReceiverV2
{
    ...
    IBCPacketStoreV2 public immutable packetStore;
    address public immutable packetHandler;
    PolicyControlledEscrowVaultV2 public escrowVault;
    PolicyControlledVoucherTokenV2 public voucherToken;
    ...
    constructor(
        ...
        packetStore = IBCPacketStoreV2(packetStore_);
        packetHandler = packetHandler_;
        if (escrowVault_ != address(0)) escrowVault = PolicyControlledEscrowVaultV2(escrowVault_);
        if (voucherToken_ != address(0)) voucherToken = PolicyControlledVoucherTokenV2(voucherToken_);
    ...
    function setEscrowVault(address escrowVault_) external onlyRole(APP_ADMIN_ROLE) {
        ...
        escrowVault = PolicyControlledEscrowVaultV2(escrowVault_);
    }
    ...
    function setVoucherToken(address voucherToken_) external onlyRole(APP_ADMIN_ROLE) {
        ...
        voucherToken = PolicyControlledVoucherTokenV2(voucherToken_);
    }
        ...
        returns (bytes32 packetId)
    {
        ...
        IBCPacketLibV2.Packet memory packet = IBCPacketLibV2.Packet({
            ...
            timeout: IBCPacketLibV2.Timeout({height: timeoutHeight, timestamp: timeoutTimestamp})
        });
