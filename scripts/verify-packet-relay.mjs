...
const OUT_FILE = resolve(process.cwd(), process.env.OUT_FILE || "proofs/besu/packet-relay-verification.json");
...
async function main() {
  ...
  if (acknowledgementHash !== expectedAckHash || receiverAckHash !== expectedAckHash) {
    throw new Error("Acknowledgement hash mismatch in IBC packet verification.");
  }
  ...
  console.log(`Deployed IBCPacketStore to ${packetStoreAddress} on chain A`);
  console.log(`Deployed BesuLightClient to ${destinationLightClientAddress} on chain B`);
  console.log(
  ...
  console.log(`Deployed IBCPacketHandler to ${destinationPacketHandlerAddress} on chain B`);
  ...
  console.log(`Saved packet verification report to ${OUT_FILE}`);
}
