...
const OUT_FILE = resolve(process.cwd(), process.env.OUT_FILE || "proofs/besu/packet-timeout-timestamp-verification.json");
...
async function main() {
  ...
  const latestDestinationBlock = await destinationProvider.getBlock("latest");
  if (!latestDestinationBlock) {
    throw new Error("Could not read latest destination block.");
  }
  const timeoutTimestamp = BigInt(latestDestinationBlock.timestamp);
  const packet = {
    sequence: 1n,
  ...
  if (!timedOut || timeoutCount !== 1n || lastTimedOutPacketId !== packetIdValue) {
    throw new Error("Timestamp-timeout state or source app timeout callback was not observed.");
  }
  ...
  console.log(
    `Opened proof-checked channel ${ethers.decodeBytes32String(SOURCE_CHANNEL_ID)} <-> ${ethers.decodeBytes32String(DESTINATION_CHANNEL_ID)}`
  );
  ...
  console.log(`Saved timestamp-timeout verification report to ${OUT_FILE}`);
}
