...
const OUT_FILE = resolve(process.cwd(), process.env.OUT_FILE || "proofs/besu/light-client-verification.json");
...
async function main() {
  ...
  const artifact = await loadArtifact("clients/BesuLightClient.sol", "BesuLightClient");
  ...
  console.log(`Deployed BesuLightClient to ${contractAddress}`);
  console.log(`Trusted header ${latest.headerUpdate.height} with state root ${trustedRoot}`);
  console.log(`Saved verification report to ${OUT_FILE}`);
}
...
main().catch(async (error) => {
  ...
  console.error(`Light-client verification failed during phase: ${error.phase}`);
  console.error(error);
  process.exit(1);
});
