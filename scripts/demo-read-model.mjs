...
export function normalizeTraceForUi(trace) {
  ...
  return {
    ...
    reverse: {
      ...(trace.reverse || {}),
      headerHeight: trace.reverse?.trustedHeight,
      headerHash: trace.reverse?.trustedHeaderHash,
      stateRoot: trace.reverse?.trustedStateRoot,
      executionStateRoot: trace.reverse?.trustedStateRoot,
      consensusHash: trace.reverse?.trustedHeaderHash,
      packetId: trace.reverse?.packetId,
      proofMode: trace.reverse?.proofMode,
    },
    misbehaviour: {
      frozen: false,
      recovered: false,
