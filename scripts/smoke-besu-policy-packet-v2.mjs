...
const OUT_FILE = resolve(process.cwd(), process.env.OUT_FILE || "proofs/besu/policy-packet-v2-smoke.json");
...
async function main() {
  ...
  const connectionKeeperArtifact = await loadArtifact("v2/core/IBCConnectionKeeperV2.sol", "IBCConnectionKeeperV2");
  ...
  const voucherArtifact = await loadArtifact("v2/apps/PolicyControlledVoucherTokenV2.sol", "PolicyControlledVoucherTokenV2");
  const escrowArtifact = await loadArtifact("v2/apps/PolicyControlledEscrowVaultV2.sol", "PolicyControlledEscrowVaultV2");
  ...
  console.log(`Opened proof-checked v2 connection ${ethers.decodeBytes32String(SOURCE_CONNECTION_ID)} <-> ${ethers.decodeBytes32String(DESTINATION_CONNECTION_ID)}`);
