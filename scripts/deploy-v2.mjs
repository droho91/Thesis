...
const CHANNEL_VERSION = ethers.hexlify(ethers.toUtf8Bytes(process.env.CHANNEL_VERSION || "ics-v2"));
const CONNECTION_PREFIX = ethers.hexlify(ethers.toUtf8Bytes(process.env.CONNECTION_PREFIX || "ibc"));
const ORDER_UNORDERED = 1;
const DEPLOY_TX_GAS_LIMIT = BigInt(process.env.DEPLOY_TX_GAS_LIMIT || process.env.V2_TX_GAS_LIMIT || "10000000");
...
async function waitForTx(tx, label) {
  const timeout = new Promise((_, reject) => {
    setTimeout(() => {
      reject(new Error(`[v2 deploy] ${label} timed out waiting for ${tx.hash}`));
    }, DEPLOY_TX_WAIT_TIMEOUT_MS);
  });
  const receipt = await Promise.race([tx.wait(), timeout]);
  if (!receipt || receipt.status !== 1) {
    throw new Error(`[v2 deploy] ${label} failed in transaction ${tx.hash}`);
  }
  return receipt;
}

async function txStep(label, send) {
  console.log(`[v2 deploy] ${label}`);
  const tx = await send();
  console.log(`[v2 deploy] ${label} tx=${tx.hash}`);
  return waitForTx(tx, label);
}

async function deployStep(label, artifact, signer, args = []) {
  console.log(`[v2 deploy] deploy ${label}`);
  const contract = await deploy(artifact, signer, args, txOptions());
  console.log(`[v2 deploy] deploy ${label} at ${await addressOf(contract)}`);
  return contract;
}
...
async function deployTransport({ chainId, owner, artifacts }) {
  ...
  const connectionKeeper = await deployStep("IBCConnectionKeeperV2", artifacts.connectionKeeper, owner, [
    chainId,
    lightClientAddress,
    ownerAddress,
  ]);
  const connectionKeeperAddress = await addressOf(connectionKeeper);
  const channelKeeper = await deployStep("IBCChannelKeeperV2", artifacts.channelKeeper, owner, [
    chainId,
    connectionKeeperAddress,
    ownerAddress,
  ]);
  const channelKeeperAddress = await addressOf(channelKeeper);
  const packetHandler = await deployStep("IBCPacketHandlerV2", artifacts.packetHandler, owner, [
    chainId,
    lightClientAddress,
    channelKeeperAddress,
    ownerAddress,
  ]);
  const packetStore = await deployStep("IBCPacketStoreV2", artifacts.packetStore, owner, [chainId]);
...
export async function runDeployV2() {
  const runtime = normalizeRuntime();
  if (!runtime.besuFirst) {
    throw new Error("deploy-v2.mjs is a Besu-first v2 entrypoint.");
  }
  ...
  console.log(`[v2] deployed Bank B stack on chainId=${chainIdB}`);
  console.log(`[v2] deployment saved to ${V2_CONFIG_PATH}`);
}
