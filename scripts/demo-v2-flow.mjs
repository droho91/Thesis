...
const LEGACY_UI_JS_PATH = resolve(process.cwd(), process.env.V2_DEMO_LEGACY_TRACE_JS || "demo/latest-run.js");
const LEGACY_UI_JSON_PATH = resolve(process.cwd(), process.env.V2_DEMO_LEGACY_TRACE_JSON || "demo/latest-run.json");
...
const BORROW_AMOUNT = ethers.parseUnits(process.env.V2_DEMO_BORROW_AMOUNT || "140", 18);
const LIQUIDATION_REPAY = ethers.parseUnits(process.env.V2_DEMO_LIQUIDATION_REPAY || "40", 18);
const SHOCKED_VOUCHER_PRICE_E18 = ethers.parseUnits(process.env.V2_DEMO_SHOCKED_VOUCHER_PRICE || "0.5", 18);
...
async function waitForTx(tx, label) {
  const timeout = new Promise((_, reject) => {
    setTimeout(() => {
      reject(new Error(`[v2 demo] ${label} timed out waiting for ${tx.hash}`));
    }, DEMO_TX_WAIT_TIMEOUT_MS);
  });
  const receipt = await Promise.race([tx.wait(), timeout]);
  if (!receipt || receipt.status !== 1) {
    throw new Error(`[v2 demo] ${label} failed in transaction ${tx.hash}`);
  }
  return receipt;
}

async function txStep(label, send) {
  console.log(`[v2 demo] ${label}`);
  const tx = await send();
  console.log(`[v2 demo] ${label} tx=${tx.hash}`);
...
async function writeTrace(trace) {
  ...
  await writeFile(OUT_JS_PATH, `window.IBCLiteLatestV2Run = ${JSON.stringify(trace, null, 2)};\n`);

  const uiCompatTrace = toLegacyUiTrace(trace);
  await writeFile(LEGACY_UI_JSON_PATH, `${JSON.stringify(uiCompatTrace, null, 2)}\n`);
  await writeFile(LEGACY_UI_JS_PATH, `window.IBCLiteLatestRun = ${JSON.stringify(uiCompatTrace, null, 2)};\n`);
}

function toLegacyUiTrace(trace) {
  return {
    version: "v2",
    generatedAt: trace.generatedAt,
    forward: {
      headerHeight: trace.forward?.trustedHeight ?? trace.forward?.commitHeight,
      headerHash: trace.forward?.trustedHeaderHash,
      packetRoot: trace.forward?.packetLeaf,
      ...
      leafIndex: null,
      proofMode: trace.forward?.proofMode ?? "storage",
    },
    lending: {
      collateralDeposited: Boolean(trace.risk?.collateralDeposited || trace.risk?.collateralAfterLiquidation),
      collateral: trace.risk?.collateralAfterLiquidation ?? trace.risk?.collateralDeposited,
      borrowed: Boolean(trace.risk?.borrowed || trace.risk?.debtAfterLiquidation),
      debt: trace.risk?.debtAfterLiquidation ?? trace.risk?.borrowed,
      repaid: Boolean(trace.risk?.repaid),
      collateralWithdrawn: Boolean(trace.risk?.collateralWithdrawn),
      completed: Boolean(trace.risk?.completed),
      liquidated: Boolean(trace.risk?.liquidationRepaid),
    },
    reverse: {
      ...
      packetRoot: trace.reverse?.packetLeaf ?? trace.timeout?.receiptStorageKey,
...
async function loadContext(config) {
  ...
  const adminA = await signerForV2(config, "A", 0);
  const adminB = await signerForV2(config, "B", 0);
  const sourceUser = await signerForV2(config, "A", Number(config.participants?.sourceUserIndex ?? 1));
  const destinationUser = await signerForV2(config, "B", Number(config.participants?.destinationUserIndex ?? 1));
  const liquidator = await signerForV2(config, "B", Number(config.participants?.liquidatorIndex ?? 2));
...
async function ensureSeededConfig(config) {
  if (!config.status?.deployed) {
    throw new Error(`No v2 deployment in ${V2_CONFIG_PATH}. Run npm run deploy:v2 first.`);
  }
  if (!config.status?.seeded || !config.participants) {
    throw new Error(`The v2 stack is not seeded yet. Run npm run seed:v2 before demo:v2.`);
  }
}
...
function baseTrace(config, ctx) {
  return {
    ...
    architecture:
      "Besu light-client-verified header updates, proof-checked IBC-like connection/channel/packet flow, and policy-controlled banking risk layer.",
    chains: {
      A: {
...
async function prepareStepContext() {
  ...
  if (!runtime.besuFirst) {
    throw new Error("demo-v2-flow.mjs is a Besu-first v2 entrypoint.");
  }
...
export async function runV2DemoStep(action, options = {}) {
  ...
  if (action === "lock") {
    ...
    const trace = await writeTracePatch(
      ...
      },
      {
        ...
        label: "Locked aBANK and committed a v2 packet",
    ...
    if (ackHash !== ethers.ZeroHash) {
      ...
      const trace = await writeTracePatch(
        ...
        },
        {
          ...
          label: "Executed v2 packet storage proof",
  ...
  if (action === "replayForward") {
    ...
    try {
      ...
      throw new Error("Replay was unexpectedly accepted by the v2 packet handler.");
    } catch (error) {
      if (!isKnownReplay(error)) throw error;
    }
    return writeTracePatch(
  ...
  if (action === "checkNonMembership") {
    return writeTracePatch(
      ...
      },
      {
        ...
        summary: "V2 timeout uses an EVM storage absence proof against the destination packet receipt slot.",
      }
    );
  }
  ...
  throw new Error(`Unknown v2 demo action: ${action}`);
}
...
async function main() {
  ...
  const trace = {
    ...
    latestOperation: {
      phase: "complete",
      label: "Completed v2 proof-checked banking demo flow",
      summary:
        "Opened/reused proof-checked connection and channel, proved a packet with Besu state proofs, ran lending valuation and liquidation, then proved timeout absence for a denied packet.",
    },
  };
  ...
  console.log("=== V2 proof-checked banking flow ===");
...
entrypoint()
  ...
  .catch((error) => {
    ...
    console.error(`demo-v2-flow failed during phase: ${error.phase}`);
    console.error(error);
    process.exit(1);
  });
