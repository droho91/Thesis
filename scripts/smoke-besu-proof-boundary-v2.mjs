...
const OUT_FILE = resolve(process.cwd(), process.env.OUT_FILE || "proofs/besu/proof-boundary-v2-smoke.json");
...
async function main() {
  ...
  const proofVerifierArtifact = await loadArtifact("v2/core/BesuEVMProofVerifierV2.sol", "BesuEVMProofVerifierV2");
  const fixtureArtifact = await loadArtifact("v2/test/StorageProofFixture.sol", "StorageProofFixture");
  ...
  if (!verificationResult) {
    throw new Error("Besu v2 proof boundary returned false for the live storage proof.");
  }
  ...
  console.log(`Deployed BesuLightClient v2 to ${lightClientAddress} on chain B`);
  console.log(`Deployed BesuEVMProofVerifierV2 to ${proofVerifierAddress} on chain B`);
  console.log(`Verified slot ${proof.storageKey} under trusted state root ${latest.headerUpdate.stateRoot}`);
  console.log(`Saved proof-boundary smoke report to ${OUT_FILE}`);
}
...
main().catch(async (error) => {
  ...
  console.error(`Proof-boundary smoke failed during phase: ${error.phase}`);
  console.error(error);
  process.exit(1);
});
