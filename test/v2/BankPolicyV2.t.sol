...
import {PolicyControlledLendingPoolV2} from "../../contracts/v2/apps/PolicyControlledLendingPoolV2.sol";

contract BankPolicyV2Test is Test {
    ...
    BankPolicyEngineV2 internal policy;
    PolicyControlledVoucherTokenV2 internal voucher;
    PolicyControlledEscrowVaultV2 internal escrow;
    PolicyControlledLendingPoolV2 internal lendingPool;
    BankToken internal canonicalAsset;
    BankToken internal debtAsset;

    function setUp() public {
        policy = new BankPolicyEngineV2(address(this));
        canonicalAsset = new BankToken("Canonical", "CAN");
        debtAsset = new BankToken("Debt", "DEBT");

        voucher = new PolicyControlledVoucherTokenV2(address(this), address(policy), "Voucher", "vCAN");
        escrow = new PolicyControlledEscrowVaultV2(address(this), address(canonicalAsset), address(policy));
        lendingPool =
            new PolicyControlledLendingPoolV2(address(this), address(voucher), address(debtAsset), address(policy), 8_000);
    ...
    function testMintVoucherWithPolicyUpdatesExposureAndRespectsCap() public {
        ...
        vm.expectRevert(abi.encodeWithSelector(PolicyControlledVoucherTokenV2.PolicyDenied.selector, policy.POLICY_VOUCHER_CAP_EXCEEDED()));
    ...
    function testDepositCollateralRequiresPolicyAllowlist() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(PolicyControlledLendingPoolV2.PolicyDenied.selector, policy.POLICY_ACCOUNT_NOT_ALLOWED())
        );
    ...
    function testBorrowRespectsAccountAndAssetCaps() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                PolicyControlledLendingPoolV2.PolicyDenied.selector, policy.POLICY_ACCOUNT_BORROW_CAP_EXCEEDED()
            )
        );
